...
class Controller_ManagerApprovalProjects extends Controller
{
    ...
    {
        $data = DB::select("select a.nama_project, b.inisial_user, c.nama_product, d.nama_ptype, e.nama_pstat, a.pketerangan_status from projects as a, users as b, products as c, projects_types as d, projects_stats as e where a.id_current_pic = b.id and a.id_product = c.id and a.id_ptype = d.id and a.id_pstat = e.id and a.id_pketerangan = 2 and (e.id = 3 or e.id = 4) order by a.waktu_assign_project asc");
